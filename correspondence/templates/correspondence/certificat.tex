...
\setupletteroptions[language=netherlands,
 ...
 \defineletterelement[layer][foot][example]%
   ...
   Je sous-signé {\sc Dr Yahyaoui}, cardiologue, certifie avoir examiné ce jour le {{ certificat.certificat_date }}   {% if patient.gender == 'F' %} Madame {% else %}Monsieur {% endif %}  {\sc {{ patient.name }} }, né{% if patient.gender == 'F' %}e{% endif %} le {{ patient.birth }} ( {{ patient.age }} ans), et déclare  {% if patient.gender == 'F' %}qu'elle{% else %}qu'il{% endif %}  présente {{ certificat.declaration }} .
