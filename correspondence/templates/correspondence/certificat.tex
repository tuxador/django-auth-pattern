...
\setupexternalfigures[directory={ {% static '/img' %}, /home/kaddour/Documents}]
   \setuplanguage [fr] [date={ {{ courrier.certificat_date }} }] % ISO 8601 date
...
\setupletteroptions[language=netherlands,
 ...
 \defineletterelement[layer][foot][example]%
   ...
   \startletter[
       toname={Au  {{ certificat.correspondant }} },
       toaddress={ {{ patient.name }} },
       subject={Certificat médical }]
   
   Je sous-signé {\sc Dr Yahyaoui}, cardiologue, certifie avoir examiné ce jour le {{ certificat.certificat_date }}   {% if patient.gender == 'F' %} Madame {% else %}Monsieur {% endif %}  {\sc {{ patient.name }} }, né{% if patient.gender == 'F' %}e{% endif %} le {{ patient.birth }}, et déclare  {% if patient.gender == 'F' %}qu'elle{% else %}qu'il{% endif %}  présente {{ certificat.declaration }} .

   
   \blank[2*big]
 {% if certificat.closing %}{{ certificat.closing }}{% else %}Dont certificat {% endif %},
   \blank[big]
   
   %\externalfigure[signature]%
   Dr Yahyaoui .M .K\\
   {\it Alger le {{ certificat.certificat_date }} }
